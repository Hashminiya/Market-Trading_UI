...
There is also a [getting started tutorial](https://vaadin.com/tutorials/getting-started-with-flow) based on this
project.

To access it directly from github, clone the repository and import the project to the IDE of your choice as a Maven
...
Tracking of local changes to the `workspace.xml` file can be prevented with
the `git update-index --assume-unchanged .idea/workspace.xml` command.
...
For a full Vaadin application example, there are more choices available also
from [vaadin.com/start](https://vaadin.com/start) page.
